...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-md border-b border-border/50">
      ...
      {/* Desktop category nav - only on main site */}
      {!isCriacaoLogo && (
        <div className="hidden md:block border-t border-border/30 bg-background/60">
            ...
            <div className="flex items-center justify-center gap-1 py-2 overflow-x-auto">
              {categoryLinks.map((cat) => (
                <Link
                  ...
                  className="text-xs font-body text-muted-foreground hover:text-foreground hover:bg-accent/50 transition-colors whitespace-nowrap px-3 py-1.5 rounded-full"
                >
                  {cat.name}
                </Link>
              ))}
            ...
            <div className="space-y-1 pb-4 border-b border-border/50">
              {categoryLinks.map((cat) => (
                <Link
                  ...
                  className="block text-sm font-body text-muted-foreground hover:text-foreground transition-colors py-2 px-2 rounded-lg hover:bg-accent/50"
                >
                  {cat.name}
                </Link>
              ))}
